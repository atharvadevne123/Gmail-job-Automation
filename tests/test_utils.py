...
def test_format_count_default_singular():
    assert format_count(1) == "1 email"


def test_format_count_default_plural():
    assert format_count(0) == "0 emails"
    assert format_count(5) == "5 emails"


@pytest.mark.parametrize("n,expected", [(0, "s"), (1, ""), (2, "s"), (100, "s")])
def test_plural_s(n, expected):
